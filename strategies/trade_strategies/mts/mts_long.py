...
class MainLongTradeStrategy(MainTradeStrategy, LongTradeStrategy):
        ...
        if self._get_profit_condition() in [1, 2, 3]:
            ...
            if self._is_f5m_closeout():
                ...
                content = log_str.format(
                    trade_time, s, sc.take_profit_cond, price, sold_pos,
                    0, sc.tp_started_point)
                logger.info(content)
            ...
            if sc.take_profit_stage == 1:
                ...
                content = log_str.format(
                    trade_time, s, sc.take_profit_cond, price,
                    sold_pos, self._get_carry_pos(), sc.tp_started_point)
                logger.info(content)
            elif sc.take_profit_stage == 2:
                if (price >=
                   self._calc_price(self.ts.open_pos_info.trade_price, 3.0, True)):
                    self.closeout(1, sp_log.format(
                        sc.take_profit_cond, '剩余全部'))
                    content = log_str.format(
                        trade_time, s, sc.take_profit_cond, price,
                        sc.tp_started_point)
                    logger.info(content)
    ...
    def _match_dk_condition(self, is_in=True) -> bool:
        logger = self.logger
        kline = self._get_last_kline_in_trade(self._d_klines)
        e9, e22, e60, macd, close, open_p, trade_time, k_date_str_short, _ =\
        ...
        if cond_number > 0:
            ...
            self._set_open_condition(
                kline, cond_number, self.ts.open_condition.daily_condition)  # type: ignore
    ...
    def _match_3h_condition(self, is_in=True) -> bool:
        ...
        if cond_number > 0:
            ...
            logger.info(content)
            self._set_open_condition(
                kline, cond_number, self.ts.open_condition.hourly_condition)  # type: ignore
    ...
    def _match_5m_condition(self, is_in=True) -> bool:
        ...
        if close > e60 and macd > 0 and diffc_60 < 1.2:
            ...
            logger.info(content)
            self._set_open_condition(
                kline, 1, self.ts.open_condition.minute_5_condition)  # type: ignore
            return True
        else:
            self._set_klines_value(
    ...
    def _set_sold_prices(self, order: Order):
        ...
        s_c.stop_loss_price = self._calc_price(
            order.trade_price, self.config.f_info.long_config.stop_loss_scale, False)
        if s_c.take_profit_cond in [1, 2, 3]:
            s_c.tp_started_point = self._calc_price(
                order.trade_price,
                self.config.f_info.long_config.profit_start_scale_1, True)
