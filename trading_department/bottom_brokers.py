...
from trade.bottom_trades import FutureTrade, FutureTradeLong,\
        FutureTradeShort
from trade.utils import TradeUtilsData


class BottomTradeBrokerManager:
    logger = LoggerGetter()
    ...
    def __init__(self,  api: TqApi, fc, direction,
        ...
        if direction:
            self._long_ftu = BottomTradeLongBroker(self.tud)
            if direction == 2:
                self._short_ftu = BottomTradeShortBroker(self.tud)
        else:
            self._short_ftu = BottomTradeShortBroker(self.tud)
