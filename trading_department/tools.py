...
from trading_department.bottom_brokers import BottomTradeBrokerManager
...
class StorageTool:
    ...
    def get_db(self, db_name='future_trade'):
        if self.is_backtest:
            return self.db_client.get_database(self.uid)
        return self.db_client.get_database(db_name)
...
class BottomTradeTool:
    logger = LoggerGetter()

    def __init__(self, direction, storageTool):
        self.direction = direction
        self.service = dao_service.BottomDBService(
            storageTool.get_db('bottom_future_trade'))
