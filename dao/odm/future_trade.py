...
class BottomOpenCondition(EmbeddedDocument):
    '''摸底策略满足开仓的条件：'''
    daily_condition = EmbeddedDocumentField(BottomIndicatorValues)
    hourly_condition = EmbeddedDocumentField(BottomIndicatorValues)
    minute_30_condition = EmbeddedDocumentField(BottomIndicatorValues)
...
class BottomSoldCondition(EmbeddedDocument):
    '''摸底策略的平仓条件'''
    # 止盈阶段
    take_profit_stage = IntField(default=0)
    # 该交易适用的止盈条件
    tp_cond = IntField(default=0)
    # 止损价格
...
class MainOpenCondition(EmbeddedDocument):
    '''主策略开仓满足的条件，包括：日线条件，3小时条件， 30分钟条件 的类型'''
    # 开仓条件
    daily_condition = EmbeddedDocumentField(MainIndicatorValues)
    hourly_condition = EmbeddedDocumentField(MainIndicatorValues)
    minute_30_condition = EmbeddedDocumentField(MainIndicatorValues)
...
class MainSoldCondition(EmbeddedDocument):
    '''存储平仓用到的条件'''
    # 止盈阶段
    take_profit_stage = IntField(default=0)
    # 该交易适用的止盈条件
...
class MainCloseVolume(TradePosBase):
    '''存储某个期货合约的平仓信息'''
...
class BottomCloseVolume(TradePosBase):
    close_type = IntField()
    close_message = StringField()
...
class MainJointSymbolStatus(Document):
    ...
    last_modified = DateTimeField()
    current_ts: TradeStatus = None
    next_ts: TradeStatus = None

    def switch_symbol(self, new_symbol: str, new_ts: TradeStatus,
        ...
        self.next_ts = new_ts
        self.trade_status = self.current_ts.trade_status
        self.last_modified = switch_time


class MJSMainStatus(MainJointSymbolStatus):
    current_ts = ReferenceField(MainTradeStatus)
    next_ts = ReferenceField(MainTradeStatus)


class MJSBottomStatus(MainJointSymbolStatus):
    current_ts = ReferenceField(BottomTradeStatus)
    next_ts = ReferenceField(BottomTradeStatus)
